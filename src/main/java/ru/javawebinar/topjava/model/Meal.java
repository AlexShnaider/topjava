...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.UPDATE, query = "UPDATE Meal m SET m.dateTime=:dateTime, " +
                "m.description=:description, m.calories=:calories WHERE m.user.id=:user_id AND m.id=:id"),
        @NamedQuery(name = Meal.ALL_SORTED, query =
                "SELECT m FROM Meal m WHERE m.user.id=:user_id ORDER BY m.dateTime DESC"),
        @NamedQuery(name = Meal.ALL_BETWEEN, query = "SELECT m FROM Meal m WHERE m.user.id=:user_id " +
